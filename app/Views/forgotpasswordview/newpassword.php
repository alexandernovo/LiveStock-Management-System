        ...
        <div class="row m-auto mt-3">
            <div class="col-md-5 border m-auto mt-5 shadow rounded px-4" style="background-color:white;">
                <div class="header text-center">
                    ...
                    <h4 class=" mt-3 text-dark">Change Your Password </br><?php if (isset($Name)) {
                                                                                echo $Name;
                                                                            } ?></h4>
                </div>
                <form method="POST">
                    <div class="form-row m-auto mb-3">
                        <input type="hidden" name="username" value="<?php if (isset($username)) {
                                                                        echo $username;
                                                                    } ?>">

                        <?php if (isset($faileds)) { ?>
                            <p class="login-error" style="margin-left:3px;"><?php echo $faileds; ?></p>
                        <?php } ?>
                        ...
                        <div class="d-flex">
                            ...
                            <span><i onclick="newpassword()" class="fafa-eye fa fa-eye"></i></span>
                        </div>

                        <?php if (isset($validation)) : ?>
                            <p class="login-error" style="margin-left:3px;"><?php echo $validation->showError('newpassword'); ?></p>
                        <?php endif; ?>
                    </div>
                    <div class="form-row m-auto">
                        <label style="font-size: 14px;">Confirm New Password</label>
                        <div class="d-flex">
                            <input type="password" id="confirmnewpassword" name="confirmnewpassword" class="form-control" placeholder="Confirm New Password" />
                            <span><i onclick="confirmnewpassword()" class="fafa-eye fa fa-eye"></i></span>
                        </div>
                        <?php if (isset($validation)) : ?>
                            <p class="login-error" style="margin-left:3px;"><?php echo $validation->showError('confirmnewpassword'); ?></p>

                        <?php endif; ?>
                    </div>
                    <div class="row m-auto my-3">
                        <button type="submit" name="change" class="btn btn-primary mb-1">Update Password</button>
                    </div>
                </form>
            </div>
