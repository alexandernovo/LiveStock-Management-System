...
<head>
  ...
  <title>Department of Agriculture Treasurer</title>
  <style>
...
<body class="users-body">
      ...
      <div class="collapse collapse-customize navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav">
          <li class="nav-items nav-item">
            <a class="nav-linkster nav-link" aria-current="page" href="<?php echo base_url(); ?>/treasurer"><i class="icons-navbar-user fa fa-home"></i>Home</a>
          </li>
          <li class="nav-items nav-item">
            <a class="nav-linkster nav-link" href="<?php echo base_url(); ?>/TreasurerPerDate?date_from=<?php echo date('Y-m-d') ?>&date_to=<?php echo date('Y-m-d', strtotime('+7 days')) ?>"><i class="icons-navbar-user fa fa-calendar"></i>Inspected</a>
          </li>
          <li class="nav-items nav-item">
            <a class="nav-linkster nav-link" href="<?php echo base_url(); ?>/PaymentHistoryDate?date_from=<?php echo date('Y-m-d') ?>&date_to=<?php echo date('Y-m-d', strtotime('+7 days')) ?>"><i class="icons-navbar-user fa fa-history"></i>History</a>
          </li>
          <li class="nav-items nav-item">
            <div class="dropdown show">
              ...
              <div class="droped dropdown-menu" aria-labelledby="dropdownMenuLink">
                <a class="drop-me dropdown-item" href="<?php echo base_url(); ?>/ManageProfileTreasurer"><i class="icons-navbar-user fa fa-cog"></i>Account Settings</a>
  ...
  <script>
    ...
    var title = "<?= session()->get('Treasurer_firstname') ?>";
  </script>
  ...
  ?>
  <script>
    var ctx = document.getElementById("this-week-payment").getContext("2d");
    var chartData = <?php echo isset($paymentTotalThisWeekPerDay) ? $paymentTotalThisWeekPerDay : "''"; ?>;
    new Chart(ctx, {
      type: "bar",
      data: {
        ...
        datasets: [{
          ...
          data: chartData.schedules ? chartData.schedules : [],
          maxBarThickness: 6
        }],
      },
      options: {
  ...
  <script>
    var ctx = document.getElementById("last-week-payment").getContext("2d");
    var chartData = <?php echo isset($paymentTotalLastWeekPerDay) ? $paymentTotalLastWeekPerDay : "''"; ?>;
    new Chart(ctx, {
      type: "bar",
      data: {
        labels: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
